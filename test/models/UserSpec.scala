...
class UserSpec extends FlatSpec with Matchers with BeforeAndAfterEach with WithInMemoryDB {
  ...
  private val users = Seq(
    User.create(None, None, "good rule", exported = false),
    User.create(None, Some("tenant"), "MO", exported = true),
    User.create(Some(index.id), Some("tenant"), "MO_DE", exported = true),
    User.create(Some(index.id), Some("tenant"), "MO_AT", exported = true)
  )
